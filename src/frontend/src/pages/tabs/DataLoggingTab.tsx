...
export default function DataLoggingTab({ isGuest }: DataLoggingTabProps) {
  ...
  const stats = {
    ...
    avgTemp: logEntries
      ...
      .reduce((sum, e) => sum + e.value, 0) / logEntries.filter((e) => e.dataType === DataType.temperature).length || 0,
    avgPressure: logEntries
      .filter((e) => e.dataType === DataType.pressure)
      .reduce((sum, e) => sum + e.value, 0) / logEntries.filter((e) => e.dataType === DataType.pressure).length || 0,
  };
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Record and track HVAC system measurements over time</CardDescription>
            </div>
            <div className="flex gap-2">
              <Button onClick={handleExportData} variant="outline" disabled={isGuest || logEntries.length === 0}>
              ...
              <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
                ...
                <DialogContent>
                  ...
                  <DialogFooter>
                    <Button onClick={handleAddEntry} disabled={addLogEntry.isPending}>
        ...
        <CardContent className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-2xl font-bold">{stats.avgTemp.toFixed(1)}°F</span>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-muted-foreground">Avg Pressure</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-center gap-2">
                  <TrendingUp className="h-4 w-4 text-primary" />
                  <span className="text-2xl font-bold">{stats.avgPressure.toFixed(1)} PSI</span>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          {/* Data Table */}
          <div>
            <h3 className="mb-4 text-lg font-semibold">Recent Entries (Last 50)</h3>
            {isLoading ? (
              ...
              <Alert>
                ...
                <AlertDescription>
                  No log entries yet. Start recording measurements to track system performance over time.
                </AlertDescription>
              </Alert>
            ) : (
                  ...
                  <TableBody>
                    {logEntries.slice(0, 50).map((entry) => (
                      <TableRow key={entry.id.toString()}>
                        ...
                        <TableCell className="text-sm">{getDataTypeLabel(entry.dataType)}</TableCell>
                        <TableCell className="text-sm font-medium">
                          {entry.value} {getDataTypeUnit(entry.dataType)}
                        </TableCell>
                        <TableCell className="text-sm">{entry.relatedJob?.toString() || 'N/A'}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
