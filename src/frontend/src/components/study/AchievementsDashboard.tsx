...
export default function AchievementsDashboard({
  ...
  const thermodynamicsCompleted =
    progressData.coreModules.thermodynamics?.completed &&
    progressData.coreModules.thermodynamics?.quizScore >= 90;

  const allEpaCompleted = Object.values(progressData.epaModules).every(
    (m) => m.completed && m.quizScore >= 70,
  );
  ...
  const allCoreCompleted = Object.values(progressData.coreModules).every(
    (m) => m.completed,
  );
  ...
  const achievements: Achievement[] = [
    {
      ...
      progress: thermodynamicsCompleted
        ? 100
        : progressData.coreModules.thermodynamics?.completed
          ? 50
          : 0,
    ...
    },
    {
      ...
      progress:
        (Object.values(progressData.epaModules).filter(
          (m) => m.completed && m.quizScore >= 70,
        ).length /
          4) *
        100,
    ...
    },
    {
      ...
      progress:
        (Object.values(progressData.coreModules).filter((m) => m.completed)
          .length /
          4) *
        100,
    ...
    <div className="space-y-6">
      ...
      <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-accent/5">
        <CardHeader>
          ...
          <CardDescription>
            You've earned {earnedAchievements.length} out of{" "}
      ...
      <Card className="border-orange-500/20 bg-gradient-to-br from-orange-500/5 to-red-500/5">
        <CardHeader>
          ...
          <CardDescription>
            Track your consecutive study days and unlock special rewards
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {[
              {
                days: 3,
                reward: "Bronze Streak",
                unlocked: progressData.longestStreak >= 3,
              },
              {
                days: 7,
                reward: "Silver Streak",
                unlocked: progressData.longestStreak >= 7,
              },
              {
                days: 14,
                reward: "Gold Streak",
                unlocked: progressData.longestStreak >= 14,
              },
              {
                days: 30,
                reward: "Platinum Streak",
                unlocked: progressData.longestStreak >= 30,
              },
            ].map((milestone) => (
              <div
                key={milestone.days}
              ...
              >
                <div className="flex items-center gap-3">
                  <Flame
                    className={`h-5 w-5 ${milestone.unlocked ? "text-orange-500" : "text-muted-foreground"}`}
                  />
                  <div>
