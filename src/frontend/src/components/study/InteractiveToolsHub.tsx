...
export default function InteractiveToolsHub({ studyMode }: InteractiveToolsHubProps) {
  ...
  const tools = [
    ...
    },
    {
      ...
      icon: Thermometer,
      description: 'Temperature measurement with superheat/subcooling calculations',
    ...
    },
    {
      ...
      icon: Droplets,
      description: 'Step-by-step recovery procedures with efficiency monitoring',
  ...
  if (selectedTool) {
    return (
      <div className="space-y-4">
        ...
        {selectedTool === 'pressure-gauge' && <DigitalPressureGauge studyMode={studyMode} />}
        {selectedTool === 'thermometer' && <DigitalThermometer studyMode={studyMode} />}
        {selectedTool === 'multimeter' && <DigitalMultimeter studyMode={studyMode} />}
        {selectedTool === 'leak-detector' && <LeakDetectorSimulator studyMode={studyMode} />}
        {selectedTool === 'airflow' && <AirflowSimulator studyMode={studyMode} />}
        {selectedTool === 'recovery-unit' && <VirtualRecoveryUnit studyMode={studyMode} />}
        {selectedTool === 'ppe-setup' && <PPESetupSimulator studyMode={studyMode} />}
        {selectedTool === 'calculator' && <SuperheatSubcoolingCalculator studyMode={studyMode} />}
      </div>
    );
  }
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Practice using professional HVAC equipment with realistic simulators. Each tool provides
            color-coded feedback and learning prompts to reinforce EPA 608 concepts.
          </p>
        </CardHeader>
        <CardContent>
