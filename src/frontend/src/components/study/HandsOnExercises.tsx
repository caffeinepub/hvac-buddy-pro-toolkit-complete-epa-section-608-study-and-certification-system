  ...
  const exercises = [
    {
      ...
      description:
        "Practice calculating temperature difference across the evaporator coil",
    },
    {
      id: "superheat" as const,
      title: "Superheat Measurement",
      description:
        "Learn to measure and calculate superheat for proper refrigerant charge",
    },
    {
...
function ExerciseContent({
  exerciseId,
  studyMode,
}: { exerciseId: Exercise; studyMode: StudyMode }) {
  ...
  const exercises = {
    deltaT: {
      ...
      instruction:
        "Calculate the temperature difference (Delta T) across the evaporator coil.",
      fields: [
        {
          name: "returnTemp",
          label: "Return Air Temperature (°F)",
          hint: "Temperature of air entering the system",
        },
        {
          name: "supplyTemp",
          label: "Supply Air Temperature (°F)",
          hint: "Temperature of air leaving the system",
        },
      ],
      calculate: (vals: Record<string, string>) => {
        const returnTemp = Number.parseFloat(vals.returnTemp);
        const supplyTemp = Number.parseFloat(vals.supplyTemp);
      ...
      feedback: {
        correct:
          "Excellent! Delta T is within the normal range of 14-22°F for proper cooling.",
        incorrect:
          "Delta T should typically be between 14-22°F. Values outside this range may indicate issues.",
      },
    ...
    superheat: {
      ...
      fields: [
        {
          ...
          hint: "Temperature at compressor suction",
        },
        {
          name: "saturationTemp",
          label: "Saturation Temperature (°F)",
          hint: "From pressure-temperature chart",
        },
      ],
      calculate: (vals: Record<string, string>) => {
        const suctionTemp = Number.parseFloat(vals.suctionTemp);
        const saturationTemp = Number.parseFloat(vals.saturationTemp);
      ...
      feedback: {
        correct:
          "Perfect! Superheat is within the typical range of 8-12°F for proper charge.",
        incorrect:
          "Superheat should typically be 8-12°F. Low superheat may indicate overcharge, high may indicate undercharge.",
      },
      example:
        "Example: Suction 50°F, Saturation 40°F → Superheat = 10°F (Good)",
    },
    subcooling: {
      ...
      fields: [
        {
          name: "condensingTemp",
          label: "Condensing Temperature (°F)",
          hint: "From high-side pressure chart",
        },
        {
          name: "liquidTemp",
          label: "Liquid Line Temperature (°F)",
          hint: "Temperature at condenser outlet",
        },
      ],
      calculate: (vals: Record<string, string>) => {
        const condensingTemp = Number.parseFloat(vals.condensingTemp);
        const liquidTemp = Number.parseFloat(vals.liquidTemp);
      ...
      feedback: {
        ...
        incorrect:
          "Subcooling should typically be 10-15°F. Low subcooling may indicate undercharge or airflow issues.",
      },
      example:
        "Example: Condensing 120°F, Liquid 108°F → Subcooling = 12°F (Good)",
    },
    airflow: {
      ...
      fields: [
        {
          name: "btuCapacity",
          label: "System Capacity (BTU/hr)",
          hint: "Rated cooling capacity",
        },
        {
          name: "deltaT",
          label: "Temperature Difference (°F)",
          hint: "Return minus supply temperature",
        },
      ],
      calculate: (vals: Record<string, string>) => {
        const btuCapacity = Number.parseFloat(vals.btuCapacity);
        const deltaT = Number.parseFloat(vals.deltaT);
      ...
      feedback: {
        correct:
          "Excellent! Airflow is adequate for the system capacity (approximately 400 CFM per ton).",
        incorrect:
          "Airflow should be approximately 400 CFM per ton of cooling. Check for restrictions or fan issues.",
      },
      example:
        "Example: 24,000 BTU/hr, 20°F ΔT → CFM = 24,000/(1.08×20) = 1,111 CFM",
    },
  };
  ...
  const allFieldsFilled = exercise.fields.every((field) =>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {showFeedback && result && (
          <Alert
            className={
              result.correct ? "border-green-500" : "border-yellow-500"
            }
          >
