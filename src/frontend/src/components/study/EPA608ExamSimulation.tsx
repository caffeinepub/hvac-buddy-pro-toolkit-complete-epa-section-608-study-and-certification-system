...
import { CheckCircle2, XCircle, Award, Clock, AlertTriangle, TrendingUp, ChevronLeft, ChevronRight, BookOpen } from 'lucide-react';
import { Progress } from '@/components/ui/progress';
import type { StudyMode } from '../../types/study';
import { Badge } from '@/components/ui/badge';
import { Switch } from '@/components/ui/switch';
import { EpaSection, QuestionDifficulty, EpaExamQuestion } from '../../types/local';
import { getRandomQuestions, shuffleAnswerOptions } from '../../data/epaQuestionBank';
...
export default function EPA608ExamSimulation({ certType, studyMode }: EPA608ExamSimulationProps) {
  ...
  const getExamConfig = () => {
    const configs = {
      ...
      universal: { duration: 90, questionCount: 30, section: EpaSection.universal },
    };
  ...
  const startExam = () => {
    ...
    if (certType === 'universal') {
      ...
      questions = [...coreQuestions, ...type1Questions, ...type2Questions, ...type3Questions];
    } else {
      // Single section exam
        ...
        <CardContent className="space-y-6">
          <Alert className="border-orange-500/50 bg-orange-500/10">
            ...
            <AlertDescription className="text-orange-900 dark:text-orange-100">
              <strong>Authentic Exam Experience:</strong> This simulation uses original, non-copyrighted questions
              that mimic the real EPA 608 certification exam. No hints or explanations will be provided during the test.
          ...
          <div className="space-y-4">
                ...
                <CardContent className="pt-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold">{config.duration} minutes</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="space-y-2">
              ...
              <ul className="list-disc pl-5 space-y-1 text-sm text-muted-foreground">
                <li>Questions are randomized from a comprehensive question pool</li>
            ...
            <div className="flex items-center justify-between rounded-lg border p-4">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  {allowBacktracking ? 'You can review previous questions' : 'Cannot return to previous questions'}
                </p>
              </div>
              <Switch
    ...
    return (
      <div className="space-y-6">
        <Card className={passed ? 'border-green-500' : 'border-red-500'}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Award className={`h-6 w-6 ${passed ? 'text-green-500' : 'text-red-500'}`} />
          ...
          <CardContent className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-3xl font-semibold mb-4">{percentage.toFixed(1)}%</p>
              {passed ? (
                <div className="space-y-2">
                      ...
                      <div key={section} className="space-y-1">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-semibold">
                            {data.correct}/{data.total} ({sectionPercentage.toFixed(0)}%)
                          </span>
                        </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  {examQuestions.map((q, idx) => {
                    ...
                    const userAnswer = selectedAnswers[idx] || 'No answer selected';
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 space-y-2">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <Badge variant="outline" className="text-xs">{q.topicCategory}</Badge>
                            </div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              <p><strong>Reference:</strong> {q.regulatoryReference}</p>
                            </div>
                          </div>
                        </div>
  ...
  const isAnswered = selectedAnswers[currentQuestion] !== undefined && selectedAnswers[currentQuestion] !== '';
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge variant="secondary" className="text-xs">{currentQ.topicCategory}</Badge>
              </div>
            </div>
        ...
        <CardContent className="space-y-6">
          <div>
            <h3 className="mb-4 text-lg font-semibold leading-relaxed">{currentQ.question}</h3>
            <RadioGroup value={selectedAnswers[currentQuestion] || ''} onValueChange={handleAnswerSelect}>
              <div className="space-y-3">
                {currentQ.options.map((option, idx) => (
                  <div key={idx} className="flex items-start space-x-3 rounded-lg border p-3 hover:bg-accent/50 transition-colors">
