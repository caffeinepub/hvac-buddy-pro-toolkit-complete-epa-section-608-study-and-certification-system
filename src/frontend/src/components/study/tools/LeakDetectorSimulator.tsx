...
import { Search, AlertTriangle, CheckCircle2, Info, Volume2, VolumeX } from 'lucide-react';

interface LeakDetectorSimulatorProps {
  studyMode: { __kind__: 'beginner' | 'expert' };
}

export default function LeakDetectorSimulator({ studyMode }: LeakDetectorSimulatorProps) {
  ...
  const getLeakStatus = () => {
    ...
    if (adjustedLevel < 20) {
      return { color: 'green', text: 'No Leak', icon: CheckCircle2, severity: 'none' };
    } else if (adjustedLevel < 50) {
      return { color: 'yellow', text: 'Minor Leak', icon: AlertTriangle, severity: 'minor' };
    } else if (adjustedLevel < 80) {
      return { color: 'orange', text: 'Moderate Leak', icon: AlertTriangle, severity: 'moderate' };
    } else {
      return { color: 'red', text: 'Major Leak', icon: AlertTriangle, severity: 'major' };
    }
  };
      ...
      <CardContent className="space-y-6">
        {isBeginner && (
          <Alert>
            ...
            <AlertDescription>
              Electronic leak detectors sense refrigerant molecules in the air. Move the probe slowly (1-2 inches
              per second) around all connections, joints, and components. Higher sensitivity detects smaller leaks
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-2xl font-bold">{adjustedLevel.toFixed(0)}%</div>
        ...
        {isBeginner && (
          <Card className="border-primary/20 bg-primary/5">
            <CardContent className="p-4">
              <h4 className="font-semibold mb-2">Leak Detection Best Practices</h4>
              <ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
                ...
                <li>Refrigerant is heavier than air - check below components</li>
                <li>Use soap bubbles to confirm electronic detector findings</li>
