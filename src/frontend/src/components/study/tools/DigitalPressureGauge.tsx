...
export default function DigitalPressureGauge({
  studyMode,
}: DigitalPressureGaugeProps) {
  ...
  const refrigerantRanges = {
    "R-410A": {
      low: { normal: [60, 80], caution: [50, 100], critical: [0, 150] },
      high: { normal: [250, 300], caution: [200, 350], critical: [0, 500] },
    },
    "R-22": {
      low: { normal: [65, 75], caution: [55, 85], critical: [0, 120] },
      high: { normal: [200, 250], caution: [150, 300], critical: [0, 400] },
    },
    "R-134a": {
      low: { normal: [25, 35], caution: [15, 45], critical: [0, 80] },
      high: { normal: [120, 150], caution: [100, 180], critical: [0, 250] },
    },
    "R-404A": {
      low: { normal: [35, 45], caution: [25, 55], critical: [0, 90] },
      high: { normal: [200, 250], caution: [150, 300], critical: [0, 400] },
    },
  };

  const currentRange =
    refrigerantRanges[refrigerant as keyof typeof refrigerantRanges][side];

  const getStatus = () => {
    if (
      pressure >= currentRange.normal[0] &&
      pressure <= currentRange.normal[1]
    ) {
      return { color: "green", text: "Normal", icon: CheckCircle2 };
    }
    if (
      pressure >= currentRange.caution[0] &&
      pressure <= currentRange.caution[1]
    ) {
  ...
  const getFeedback = () => {
    if (side === "low") {
      ...
      if (pressure > currentRange.normal[1]) {
      ...
      <CardContent className="space-y-6">
        {isBeginner && (
          <Alert>
            ...
            <AlertDescription>
              Pressure gauges measure refrigerant pressure in PSI. Low side
              (blue) measures suction pressure, high side (red) measures
              discharge pressure. Proper pressures indicate correct system
              operation.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={side}
              onValueChange={(v) => setSide(v as "low" | "high")}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <Card className="border-primary/20 bg-primary/5">
            <CardContent className="p-4">
              ...
              <p className="text-sm text-muted-foreground">
                Normal pressure ranges vary by refrigerant type and ambient
                temperature. R-410A operates at 60% higher pressures than R-22.
                Always use manufacturer specifications and pressure-temperature
